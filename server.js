...
function loadDatabase() {
  ...
  let data =  yaml.safeLoad(fs.readFileSync('backup_database.yml', 'utf8'));
...
function createComment(url, request) {
  //copied from createArticle and edited for comment
  const requestComment = request.body && request.body.comment;
  const response = {};

  if (requestComment && database.articles[requestComment.articleId] && requestComment.body && requestComment.username && database.users[requestComment.username]) {
    const comment = {
...
function updateComment(url, request) {
  //copied from updateArticle and edited for comment
  const id = Number(url.split('/').filter(segment => segment)[1]);
